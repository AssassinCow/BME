...
def _config(inference_batch_size: int, include_chunks: bool) -> dict[str, object]:
    training = {
        "random_seed": 2026,
        "max_epochs": 32,
...
def test_completed_state_checkpoint_migration_preserves_weights(tmp_path) -> None:
    ...
    torch.save(_checkpoint(source_config, epoch=5, patience=0), source_state / "best.pt")
    torch.save(_checkpoint(source_config, epoch=13, patience=4), source_state / "last.pt")
    (source_state / "normalization.json").write_text("{}", encoding="utf-8")
    pd.DataFrame({"epoch": [5, 13]}).to_csv(source_state / "history.csv", index=False)
    ...
    assert migration["allowed_runtime_changes"] == {
