...
from bme_eating.training.hierarchical_trainer import (
    build_fold_candidates,
    load_hierarchical_inputs,
)


def main() -> None:
    parser = argparse.ArgumentParser(description="Build leakage-safe hierarchical candidates")
    ...
    if args.fresh:
        raise SystemExit("Candidate generation continues an existing state run; use --resume")
